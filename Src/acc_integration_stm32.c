 ...
 */
static gpio_config_t saved_gpio_status[5];
...
{
	RTC_AlarmTypeDef alarm = { {0}, 0, 0, 0, 0, 0};
...
static void gpio_suspend(void)
{
	...
	saved_gpio_status[2].RCC_GPIO_CLK_ENABLE = get_gpio_port_clock(GPIOC);
	saved_gpio_status[3].RCC_GPIO_CLK_ENABLE = get_gpio_port_clock(GPIOD);
	saved_gpio_status[4].RCC_GPIO_CLK_ENABLE = get_gpio_port_clock(GPIOF);
	...
	save_gpio_bank(GPIOC, &saved_gpio_status[2]);
	save_gpio_bank(GPIOD, &saved_gpio_status[3]);
	save_gpio_bank(GPIOF, &saved_gpio_status[4]);
	...
	GPIO_InitStructOff.Pin &= (uint16_t) ~(A111_SPI_SCK_Pin |
	                                       A111_SPI_MISO_Pin |
	                                       A111_SPI_MOSI_Pin |
	                                       A111_ENABLE_Pin |
	                                       MISC_GPIO0_Pin |
	...
	GPIO_InitStructOff.Pin &= (uint16_t) ~(A111_CS_N_Pin |
	                                       A111_CTRL_Pin |
	                                       A111_SENSOR_INTERRUPT_Pin |
	                                       I2C_ADDRESS_Pin |
	                                       MISC_GPIO1_Pin |
	                                       PMU_ENABLE_Pin |
	                                       PS_ENABLE_Pin);
	...
	HAL_GPIO_Init(GPIOC, &GPIO_InitStructOff);
	HAL_GPIO_Init(GPIOD, &GPIO_InitStructOff);
	HAL_GPIO_Init(GPIOF, &GPIO_InitStructOff);
...
static void gpio_resume(void)
{
	...
	restore_gpio_bank(GPIOC, &saved_gpio_status[2]);
	restore_gpio_bank(GPIOD, &saved_gpio_status[3]);
	restore_gpio_bank(GPIOF, &saved_gpio_status[4]);
}
...
static void acc_integration_prepare_stop_1(void)
{
	...
	prepare_saved_i2c_state = hi2c1.State;
	HAL_I2C_DeInit(&hi2c1);
	HAL_SPI_DeInit(&A111_SPI_HANDLE);
	prepare_saved_uart1_rx_state = huart1.RxState;
...
static void acc_integration_resume_stop_1(void)
{
	...
	if (prepare_saved_uart1_rx_state == HAL_UART_STATE_BUSY_RX)
	{
		...
		{
			HAL_UART_Receive_IT(&huart1, huart1.pRxBuffPtr, huart1.RxXferSize);
		}
	}
